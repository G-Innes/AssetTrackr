...
describe('getAllTransactionsForAsset', () => {
  ...
  it('should return 404 if user is not found', async () => {
    ...
    const response = await request(app).get(`/user/${nonExistentUserId}/transactions/${asset.id}`);
  ...
  it('should return 404 if asset is not found', async () => {
    ...
    const response = await request(app).get(`/user/${user.id}/transactions/${nonExistentAssetId}`);
...
describe('getTransactionsByType', () => {
  it('should return all transactions of a specific type for a specific user', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body).toHaveLength(1);
    const allTransactionsAreBuy = response.body.every((transaction: any) => transaction.transactionType === TransactionType.BUY);
...
it('should return 404 if user is not found', async () => {
  ...
  const response = await request(app).get(`/user/${nonExistentUserId}/transactions/type/buy`);
...
it('should return 400 if invalid transaction type is provided', async () => {
  ...
  expect(response.body).toHaveProperty('message', 'Invalid transaction type: invalidType');
});
