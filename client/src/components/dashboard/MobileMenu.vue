...
<template>
  ...
  <button
    type="button"
    class="glass-card fixed right-4 top-4 z-40 rounded-full p-2 text-dark-200 hover:text-white shadow-glow-white lg:hidden"
    @click="toggleMenu"
  >
    <span class="sr-only">{{ isOpen ? 'Close menu' : 'Open menu' }}</span>
    <svg v-if="!isOpen" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
  ...
  >
    ...
    <!-- Side drawer -->
    <div class="glass-card fixed inset-y-0 left-0 z-40 w-full max-w-xs overflow-y-auto border-r border-white/10 bg-glass-gradient px-6 py-4 backdrop-blur-md sm:max-w-sm">
